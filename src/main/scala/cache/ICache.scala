...
class iCache(
  ...
  lowLevelMem.AWDOMAIN := 0.U
  lowLevelMem.AWSNOOP := 0.U
  lowLevelMem.AWBAR := 0.U
  ...
  lowLevelMem.ACREADY := true.B
  lowLevelMem.CRVALID := true.B
  lowLevelMem.CRRESP := 0.U
  lowLevelMem.CDVALID := false.B
  lowLevelMem.CDDATA := 0.U
  lowLevelMem.CDLAST := false.B
}
